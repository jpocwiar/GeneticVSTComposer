...
GeneticMelodyGenerator::GeneticMelodyGenerator(const std::string& scale, const std::pair<int, int>& noteRange,
    ...
    // Przyk�adowe wype�nienie scale_notes - to b�dzie wymaga�o prawdziwej implementacji
...
void GeneticMelodyGenerator::set_coefficients(const std::map<std::string, float>& mu_values,
    ...
    // Ustaw warto�ci domy�lne, je�li nie przekazano �adnych map
...
void GeneticMelodyGenerator::mutate(std::vector<int>& melody) {
    ...
    // Poni�ej pokazuj� jedn� z mutacji jako przyk�ad. Pozosta�e mutacje powiniene� zaimplementowa� analogicznie.

    // Pierwsza mutacja: zamiana dw�ch nut na interwa�
    if (prob_dist(rng) < MUTATION_RATE && !melody.empty()) {
        ...
        } while (second_note_index == first_note_index); // Zapewniamy r�ne indeksy

        int interval = interval_dist(rng);
        melody[second_note_index] = melody[first_note_index] + interval;
        // Ustawienie noty w obr�bie dozwolonego zakresu
    ...
    if (prob_dist(rng) < MUTATION_RATE && !melody.empty()) {
        ...
        // Transpozycja nut w obr�bie fragmentu
    ...
    if (std::uniform_real_distribution<float>(0.0, 1.0)(rng) < MUTATION_RATE && !valid_indices.empty()) {
        int extend_index = valid_indices[std::uniform_int_distribution<int>(0, valid_indices.size() - 1)(rng)];

        if (extend_index == 0) {
            melody.insert(melody.begin() + 1, -2); // Insert an extension at the beginning
            melody.pop_back(); // Remove the last note to keep the melody length the same
        }
        else {
            // Swap the chosen note with an extension
...
std::vector<std::vector<int>> GeneticMelodyGenerator::generate_population(int note_amount) {
    ...
    std::uniform_int_distribution<int> note_dist(0, NOTES.size() - 1); // Dystrybucja dla indeks�w NOTES
    std::uniform_int_distribution<int> change_dist(-12, 12); // Dystrybucja dla zmiany wysoko�ci d�wi�ku

    for (int i = 0; i < populationSize; ++i) {
        std::vector<int> individual;
        individual.push_back(NOTES[note_dist(rng)]); // Losowy pierwszy d�wi�k

        for (int j = 1; j < note_amount; ++j) {
            int change = change_dist(rng);
            int next_note = individual.back() + change;

            // Upewnij si�, �e next_note jest w zakresie NOTES
...
std::pair<std::vector<int>, std::vector<int>> GeneticMelodyGenerator::crossover(const std::vector<int>& parent1, const std::vector<int>& parent2) {
    std::uniform_int_distribution<int> dist(1, parent1.size() - 2); // Zapobiegamy tworzeniu skrajnych przypadk�w
    int index = dist(rng);
...
std::pair<float, float> GeneticMelodyGenerator::fitness_scale_and_chord(const std::vector<int>& melody) {
    ...
    float chord_conformance_score = 0.0; // We'll leave this as 0 until chord checking is implemented
...
float GeneticMelodyGenerator::fitness(const std::vector<int>& melody) {
